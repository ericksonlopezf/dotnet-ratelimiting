...
foreach ($tf in $textFiles) {
    ...
    if ($rawText -match "Â©") {
        Write-Host "  ❌ Corrupted UTF-8 copyright sequence (Â©) detected in $($tf.FullName)" -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Corrupted UTF-8 copyright sequence in $($tf.FullName)") } else { $Violations++ } }
        if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
        $encodingErrors++
    }
}
