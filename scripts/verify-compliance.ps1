...
<#
.SYNOPSIS
    Architecture & Quality Standards Compliance Verification Script for dotnet-ratelimiting.
.DESCRIPTION
...
Write-Host "  Repository: dotnet-ratelimiting" -ForegroundColor Cyan
Write-Host "==================================================" -ForegroundColor Cyan
...
$docsFiles = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.md" -ErrorAction SilentlyContinue | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|BenchmarkDotNet\.Artifacts|StrykerOutput|node_modules|\.git)[\\/]'
}
$badDocNames = 0
$exemptNames = @('README.md', 'SECURITY.md', 'CODE_OF_CONDUCT.md', 'CONTRIBUTING.md', 'SUPPORT.md', 'CHANGELOG.md', 'PULL_REQUEST_TEMPLATE.md')
if ($docsFiles) {
    foreach ($doc in $docsFiles) {
        $filename = $doc.Name
        if ($filename -notin $exemptNames -and ($filename -cne $filename.ToLower() -or $filename -match "_")) {
...
$srcCsFiles = Get-ChildItem -Path (Join-Path $RootDirectory "src") -Recurse -Filter "*.cs" -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notmatch "[\\/](bin|obj)[\\/]" }
$obsoleteCount = 0
if ($srcCsFiles) {
    foreach ($cs in $srcCsFiles) {
        $lines = Get-Content $cs.FullName
        for ($i = 0; $i -lt $lines.Count; $i++) {
            if ($lines[$i] -match "^\s*\[Obsolete" -and $lines[$i] -notmatch "^\s*//") {
...
if (-not $strykerConfigFiles -or $strykerConfigFiles.Count -eq 0) {
    ...
    foreach ($sf in $strykerConfigFiles) {
        ...
        ($_.Name -match '^stryker-.+-config\.json$' -or ($_.Name -eq "stryker-config.json" -and $strykerConfigFiles.Count -eq 1))
    }
...
# -----------------------------------------------------------------------------
# Implicit Usings Uniform Policy Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Implicit Usings] Checking uniform Implicit Usings policy across all projects..." -ForegroundColor Yellow
$allCsproj = Get-ChildItem -Path $targetRoot -Recurse -Filter "*.csproj" -ErrorAction SilentlyContinue | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj)[\\/]'
}
$implicitUsingErrors = 0
foreach ($proj in $allCsproj) {
    $content = Get-Content $proj.FullName -Raw
    if ($content -match '<ImplicitUsings>\s*disable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Prohibited '<ImplicitUsings>disable</ImplicitUsings>' in $($proj.FullName)" -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Prohibited ImplicitUsings disable in $($proj.FullName)") } else { $Violations++ } }
        if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
        $implicitUsingErrors++
    }
}
if ($implicitUsingErrors -eq 0) {
    Write-Host "  ✅ Uniform Implicit Usings policy ('enable') verified across all $($allCsproj.Count) projects." -ForegroundColor Green
}

# -----------------------------------------------------------------------------
# Encoding & Character Integrity Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Character Integrity] Checking UTF-8 character integrity across repository..." -ForegroundColor Yellow
$checkedExts = @(".cs", ".md", ".yml", ".yaml", ".ps1", ".props", ".targets")
$textFiles = Get-ChildItem -Path $targetRoot -Recurse -File -ErrorAction SilentlyContinue | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|BenchmarkDotNet\.Artifacts|StrykerOutput|node_modules|\.git)[\\/]' -and
    $checkedExts -contains $_.Extension
}
$encodingErrors = 0
foreach ($tf in $textFiles) {
    $rawText = [System.IO.File]::ReadAllText($tf.FullName, [System.Text.Encoding]::UTF8)
    if ($rawText -match '\u00C2\u00A9') {
        $corruptedSeq = "$([char]0x00C2)$([char]0x00A9)"
        Write-Host "  ❌ Corrupted UTF-8 copyright sequence ($corruptedSeq) detected in $($tf.FullName)" -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Corrupted UTF-8 copyright sequence in $($tf.FullName)") } }
        if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
        $encodingErrors++
    }
}
if ($encodingErrors -eq 0) {
    Write-Host "  ✅ UTF-8 encoding integrity verified across $($textFiles.Count) files." -ForegroundColor Green
}
